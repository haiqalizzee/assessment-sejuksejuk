    ...
    >
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg text-blue-900">{job.id}</CardTitle>
          <Badge className={isCompleted ? "bg-green-100 text-green-800" : "bg-blue-100 text-blue-800"}>
            {isCompleted ? "Completed" : job.serviceType}
          </Badge>
        </div>
        <CardDescription className="text-blue-600">{job.customerName}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <div className="flex items-start gap-2">
          ...
          <p className="text-sm text-gray-700">{job.problemDescription}</p>
        </div>
